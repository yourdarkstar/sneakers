...
function Drawer({ onClose, onRemove, items = [], opened }) {
  ...
  const onClickOrder = async () => {
    try {
      setIsLoading(true);
      const { data } = await axios.post('https://642fc67cc26d69edc8832fe1.mockapi.io/orders', {
        items: cartItems,
      });
      setOrderId(data.id);
      ...
      for (let i = 0; i < cartItems.length; i++) {
        ...
        await axios.delete('https://642e9f0f2b883abc6412aa07.mockapi.io/cart/' + item.id);
        await delay(1000);
      }
    } catch (error) {
      ...
      <div className={styles.drawer}>
        ...
        {items.length > 0 ? (
          ...
          <Info
            ...
            image={isOrderComplete ? {completeOrder} : 'img/empty-cart.jpg'}          />
        )}
      </div>
    </div>
  );
